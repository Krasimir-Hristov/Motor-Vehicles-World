...
export default function ListingItem({ listing, id, onEdit, onDelete }) {
  ...
  shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
    ...
    { onDelete && (
        <FaTrash className='absolute bottom-2 right-2 text-black cursor-pointer text-red-700'
        onClick={() => onDelete(listing.id)}
        />
    ) }

{ onEdit && (
        <MdEdit className='absolute bottom-2 right-10 text-black cursor-pointer text-black h-4'
